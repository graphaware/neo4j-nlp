...
public class WorkflowProcessorProcedure extends AbstractDSL {
    ...
    public Stream<WorkflowItemInfo> available() {
        try {
            Set<WorkflowItemInfo> workflowProcessors = getPipelineManager().getPipelineProcessorClasses();
    ...
    public Stream<WorkflowInstanceItemInfo> create(@Name(value = "name") String name,
        ...
        try {
            WorkflowProcessor workflowInput = getPipelineManager().createPipelineProcessor(name, className, parameters);
    ...
    public Stream<WorkflowInstanceItemInfo> list() {
        try {
            Set<WorkflowInstanceItemInfo> workflowProcessors = getPipelineManager().getPipelineProcessorInstances();
            return workflowProcessors.stream();
        } catch (Exception e) {
            LOG.error("ERROR in PipelineProcessorProcedure", e);
            throw new RuntimeException(e);
